...
import { Modal, StyleSheet, Text, View, TouchableOpacity, Dimensions, TouchableWithoutFeedback, Vibration } from 'react-native';
...
/**
 * react-native-web 전용 마우스 이벤트를 전달하기 위한 헬퍼.
 * app.json의 platforms에 'web'이 포함되어 있어 웹에서는 드래그에 필요하지만,
 * React Native의 View 타입에는 onMouseEnter/onMouseLeave가 없습니다.
 * 네이티브에서는 무시되고 터치 이벤트가 대신 동작합니다.
 */
const webMouseProps = (handlers: Record<string, () => void>) =>
...
export const CalendarModal: React.FC<CalendarModalProps> = ({
  ...
  // 시트 하단 여백이 `paddingBottom: 34` 로 고정돼 있었습니다. iOS 홈 인디케이터
  // 기준이라 안드로이드 3버튼 내비게이션(48dp)에는 모자라, **'취소'·'일정 저장'
  // 버튼 아래쪽이 내비게이션 바에 잘렸습니다.** 버튼을 누르려다 바로 위의
  // 시간 슬롯이 눌리는 오조작으로 이어집니다.
      ...
      <TouchableWithoutFeedback onPress={onClose}>
        <View style={styles.overlay}>
          <TouchableWithoutFeedback>
            <View style={[styles.bottomSheet, { paddingBottom: Math.max(insets.bottom, 16) + 18 }]}>
              {/* 노치 핸들 */}
              ...
              <View
                ...
                {...webMouseProps({ onMouseLeave: () => handleTouchEnd() })}
              >
                {Array(24)
                  .fill(null)
                  .map((_, index) => {
                    ...
                    return (
                      <View
                        ...
                        {...webMouseProps({ onMouseEnter: () => handleSlotTouchMove(index) })}
                      >
                        <Text
                          style={[
                            styles.gridItemText,
