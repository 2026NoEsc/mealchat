...
import { Calendar as CalendarIcon, Sparkles, CheckCircle, Clock, Check, Users, ChevronLeft, ChevronRight, Search, Plus, X, UserPlus, Pencil, Trash2 } from 'lucide-react-native';
...
const fetchKakaoPlaces = async (keyword: string, lat: number, lng: number) => {
  if (!KAKAO_REST_API_KEY) {
    return getMockKakaoPlaces(keyword, lat, lng);
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('Kakao API request failed:', response.status);
      return getMockKakaoPlaces(keyword, lat, lng);
    }
...
const getMockKakaoPlaces = (keyword: string, lat: number, lng: number) => {
  const parts = keyword.split(' ');
  const station = parts[0] || '서면역';
  const cat = parts[1] || '맛집';

  const namesMap: Record<string, string[]> = {
    '한식': ['다담 한식당', '조선초가한끼', '수라선'],
    '일식': ['스시효', '우동 카덴', '미도인'],
    '양식': ['바피아노', '라그릴리아', '살바토레쿠오모'],
    '중식': ['딤딤섬', '싱카이', '크리스탈 제이드'],
    '고기': ['하남돼지집', '육전식당', '교대이층집'],
    '치킨': ['깐부치킨', '생활맥주', '아웃닭'],
    '카페': ['블루보틀커피', '테라로사', '아티제'],
    '술집': ['역전할머니맥주', '금별맥주', '포차어게인']
  };

  const names = namesMap[cat] || ['로컬 인기 식당', '숨겨진 맛집', '인기 장소'];
  const isSeoul = lat > 36;

  return names.map((name, idx) => {
    const distance = Math.round(150 + idx * 120);
    const address = isSeoul
      ? `서울특별시 강남구 역삼동 ${200 + idx * 25}`
      : `부산광역시 부산진구 부전동 ${100 + idx * 15}`;
    const roadAddress = isSeoul
      ? `강남구 테헤란로 ${10 + idx * 4}길`
      : `부산진구 중앙대로 ${700 + idx * 8}`;
    ...
    return {
      ...
      x: (lng + 0.001 * idx).toString(),
      y: (lat + 0.0008 * idx).toString(),
...
const getPlaceDetailsByCategory = (category: string, placeName: string) => {
  ...
  const reviewsCount = 45 + (code % 350);
  ...
  return {
    rating: `⭐ ${rating} (${reviewsCount}개 리뷰)`,
    menu: matched.menu,
    price: matched.price,
...
export const ScheduleGrid: React.FC<ScheduleGridProps> = ({
  ...
  const [coordinationStartDate, setCoordinationStartDate] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    activeLocs.forEach(p => {
      const lat = p.start_latitude || 37.5665;
  ...
  const syncNativeCalendar = async () => {
    ...
    try {
      ...
      if (status !== 'granted') {
        setSyncing(false);
        Alert.alert('권한 오류', '캘린더 접근 권한이 거부되었습니다. 설정에서 권한을 허용해 주세요.');
        return;
      }
        ...
        if (room.confirmed_slot.includes('~')) {
          ...
          if (room.meeting_date === date) {
            ...
            if (timeMatch) {
              ...
              // 약속 시간과 그 다음 시간을 바쁨으로 표시 (1시간)
              if (timeHour === slotHour) return true; // 약속 시간
              if (timeHour === slotHour + 1 && slotMinute > 0) return true; // 다음 시간 (분이 있으면)
            }
          }
        }
      }
  ...
  const handleDragStart = (e: any) => {
    ...
    gridRef.current?.measure((x, y, width, height, pageX, pageY) => {
      ...
      // Column 0 is time labels (60px). Day columns are 80px each.
    ...
    return activeHeatmapParticipants.filter(p => {
      ...
      if (isCoordination) {
        if (p.id === myProfile?.id) {
          return mySchedule[date]?.includes(time);
        }
        return p.schedule?.[date]?.includes(time);
      } else {
  ...
  const getMidPointRecommendation = () => {
    ...
    const travelList = activeLocs.map(p => {
      const pLat = p.start_latitude || 37.5665;
      const pLng = p.start_longitude || 126.9780;
    ...
    const hotplaces = (cached && cached.length > 0)
      ? cached
      : matchedCategories.map((cat, idx) => {
        ...
        const names = namesMap[cat] || ['맛있는 식당', '감성 넘치는 카페', '로컬 핫플레이스'];
        const chosenName = names[idx % names.length];
        const isSeoul = closestHub.latitude > 36;
        const address = isSeoul
          ? `서울특별시 강남구 역삼동 ${200 + idx * 25}`
          : `부산광역시 부산진구 부전동 ${100 + idx * 15}`;

        const details = getPlaceDetailsByCategory(cat, chosenName);
        return {
          category: cat,
          name: `${chosenName} ${closestHub.name}점`,
          basicInfo: `${address} (약 ${(150 + idx * 120)}m)`,
          description: `메이트들이 선호하는 인기 매장`,
          rating: details.rating,
          menu: details.menu,
          price: details.price,
  ...
  const handleCreateRoomFromCoordination = async () => {
    ...
    try {
      ...
      // 선택한 시작일이 없으면 기본값(selectedDate) 사용
      const finalStartDate = coordinationStartDate || selectedDate;
        ...
        <View style={{ marginVertical: 8 }}>
          {confirmedRooms.map((confirmedRoom) => {
            const isMemoVisible = (() => {
              ...
              if (visibility === 'best') {
                return friendsWhoMarkedMeBest.includes(confirmedRoom.memo_author_id);
              }
              return true;
            })();
            ...
            return (
              ...
              >
                ...
                {myId && (
                  <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 4, pt: 6, borderTopWidth: 1, borderTopColor: 'rgba(0,0,0,0.05)' }}>
                    <TouchableOpacity
          ...
          {loadingNotes ? (
            <ActivityIndicator size="small" color={THEME.primary} style={{ py: 10 }} />
          ) : dayNotes.length > 0 ? (
            dayNotes.map(note => {
              ...
              return (
                ...
                >
                  ...
                  {isMine && (
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 12, marginTop: 8, pt: 8, borderTopWidth: 1, borderTopColor: '#EEE7E8' }}>
                      <TouchableOpacity
                        style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}
                        onPress={() => {
                          startEditNote(note);
    ...
    return (
      <View style={styles.timeSelectorCard}>
        ...
        <ScrollView
          ...
          style={styles.timetableHorizontalScroll}
      ...
      {!isConfirmed && (
        <View style={styles.aiCard}>
          ...
          {isCoordination ? (
            <View>
              {/* Debug Info */}
              <Text style={{ fontSize: 10, color: THEME.textMuted, marginBottom: 8, padding: 8, backgroundColor: THEME.surfaceDarker, borderRadius: 4 }}>
            ...
            recommendations.length > 0 ? (
              <View style={{ gap: 8 }}>
                {recommendations.map((rec, idx) => {
                  const formattedDate = new Date(rec.date).toLocaleDateString('short', {
                    month: 'short',
                    day: 'numeric',
                    weekday: 'short',
                  });
                            ...
                            {loadingAIDetails ? (
                              ...
                              <>
                                <Text style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>
                                  {place.category} · {parseReviewsCount(place.rating)}
                                </Text>
      ...
      {isGlobal && (
        <View style={{ marginTop: 16 }}>
          ...
          {activeRooms.length > 0 ? (
            ...
            <Text style={styles.noRoomsText}>예정된 약속이 없습니다. [일정 조정]에서 개설해 보세요.</Text>
          )}
        </View>
      )}
