...
import { Camera, User, Heart, ShieldAlert, Sparkles, Check, Users, Search, Trash2, Calendar, X, ChevronDown, ChevronUp, ChevronRight } from 'lucide-react-native';
import * as ImagePicker from 'expo-image-picker';
import * as Location from 'expo-location';
import MapView, { Marker } from 'react-native-maps';
import { supabase } from '../lib/supabaseClient';
import { THEME } from '../lib/theme';
import type { PersonalData, Profile, Follow, ScheduleAvailability, PrivacySettings } from '../lib/types';
...
export const ProfileSetup = forwardRef<any, ProfileSetupProps>(({
  ...
  const handleGetCurrentLocation = async (targetField: 'startLocation' | 'preferredLocation') => {
    try {
      const { status } = await Location.requestForegroundPermissionsAsync();
      if (status !== 'granted') {
        Alert.alert('권한 거부', '실제 기기 위치를 가져오기 위해 위치 권한이 필요합니다.');
        return;
      }
  ...
  const [startLatitude, setStartLatitude] = useState(initialData?.start_latitude || 37.5665);
  const [startLongitude, setStartLongitude] = useState(initialData?.start_longitude || 126.9780);
  ...
  const saveTastePreferenceData = async () => {
    ...
    try {
      ...
      }, userTag, avatarUrl, startLocationName.trim(), startLatitude, startLongitude);
          ...
          <ScrollView contentContainerStyle={styles.resultContainer} showsVerticalScrollIndicator={false}>
              ...
              {expandedSection === 'allergy' && (
                <View style={styles.accordionContent}>
                  <View style={styles.tagSelector}>
                    {['갑각류', '견과류', '메밀', '밀', '복숭아', '우유', '조개류', '토마토', '계란'].map(opt => {
            ...
            <View style={styles.accordionCard}>
              ...
              {expandedSection === 'disease' && (
                <View style={styles.accordionContent}>
                  <View style={styles.tagSelector}>
                    {['당뇨', '고혈압', '통풍', '역류성식도염', '만성 위염', '고지혈증'].map(opt => {
        ...
        <View>
          ...
          {activeSettingSection === 'profile' && (
            <View>
                ...
                {isUploadingImage && (
                  <View style={{ marginTop: 8, alignItems: 'center' }}>
                    ...
                    <View style={{ width: '100%', height: 4, backgroundColor: '#E5E7EB', borderRadius: 2, overflow: 'hidden' }}>
                      <View style={{ width: `${uploadProgress}%`, height: '100%', backgroundColor: avatarColor, transition: 'width 0.2s' }} />
                    </View>
                  </View>
                )}
              ...
              {/* 나만의 프로필 이미지 만들기 */}
              <Text style={styles.sectionLabel}>나만의 이모지 프로필 커스텀</Text>
      ...
      >
        <SafeAreaView style={{ flex: 1, backgroundColor: '#ffffff' }}>
          ...
          <View style={{ padding: 16, borderTopWidth: 1, borderTopColor: '#e2e8f0', backgroundColor: '#f8fafc' }}>
            ...
            <TouchableOpacity
              style={{ backgroundColor: THEME.primary, paddingVertical: 12, borderRadius: 8, alignItems: 'center', shadowColor: THEME.primary, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 3, elevation: 2 }}
              onPress={async () => {
                ...
                if (mapTargetField === 'startLocation') {
                  ...
                  Alert.alert('성공', '지도의 선택된 좌표와 주소가 출발지로 설정되었습니다!');
                } else {
                  try {
                    const address = await Location.reverseGeocodeAsync({
                      latitude: targetCoords.latitude,
                  ...
                  Alert.alert('성공', '지도의 선택된 주소가 선호 모임 장소로 설정되었습니다!');
                }

                setShowMapModal(false);
              }}
            >
              <Text style={{ color: '#ffffff', fontSize: 13, fontWeight: 'bold' }}>이 위치로 설정하기</Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </Modal>
